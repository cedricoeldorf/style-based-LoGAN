...
def generate_image_grid(sess, op):
    ...
    for r in random_inputs:
        for t in sample_y:
            ...
            ax = plt.subplot(gs[i])
            i += 1
            img = np.array(x.tolist()).reshape(60, 60)
...
def train(resolution,train_model=True):
    """
    ...
    with tf.Session() as sess:
        if train_model:
            ...
            x_l, y_l = next_batch(df_test, batch_size,0)
            for i in range(n_epochs):
                ...
                for b in range(1, n_batches + 1):
                    ...
                    batch_x_ul, _ = next_batch(df,batch_size,start)
...
if __name__ == '__main__':
    ...
    train(resolution,train_model=args.train)
